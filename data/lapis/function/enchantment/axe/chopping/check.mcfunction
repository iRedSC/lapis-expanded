...
execute if predicate lapis:enchantment/holding_chopping_tool store result score $level lapis.temp run data get entity @s SelectedItem.components."minecraft:enchantments".levels."lapis:axe/chopping"
...
execute if score $success lapis.temp matches 1 if predicate lapis:enchantment/holding_chopping_tool as @e[tag=lapis.temp] at @s if block ~ ~ ~ #minecraft:air run function lapis:enchantment/axe/chopping/start
